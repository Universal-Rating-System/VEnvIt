...
function InitVirtualEnvironment {
    ...
    if (-not $env:VENV_ENVIRONMENT -or -not $env:VENVIT_DIR -or -not $env:SECRETS_DIR -or -not $env:PROJECTS_BASE_DIR -or -not $env:VENV_BASE_DIR) {
        ShowEnvVarHelp
        return
    }
...
Write-Host "Initialize the $args[0] virtual environment" -ForegroundColor Blue
